import pytest
from books.models import Book, Shelf, Review
...
def test_added_book_in_shelf(auth_client, user, book):
    response = auth_client.post(f"/api/shelf/", {"book": book.id}, format="json")
...
@pytest.mark.parametrize("book", [
    pytest.param({"title": "Смерть на нілі", "author": "Агата Крісті", "year": 1449, "total_pages": 300},  id="1449 year"),
    pytest.param({"title": "Витончене мистецтво", "author": "Марк Менсон", "year": 3001, "total_pages": 300}, id="3001 year"),
    pytest.param({"title": "Пошук сенсу", "author": "Віктор Франкл", "year": -500, "total_pages": 300}, id="-500 year") ])
@pytest.mark.django_db
